var input    = document.getElementById('myInput'),
   ...
   $('#generate').click(function(){
            ...
            for(var i = 0 ; i < num ;i++ ){
                ...
                if(arr.indexOf(randNum) == -1)
                {
                    ...
                    but.onclick = function(e){
						...
						localStorage.setItem(e.target.textContent,JSON.stringify(val));
                        data = JSON.parse(localStorage.getItem(e.target.textContent));
    ...
    window.addEventListener('load',function(event){
        let val= { type : 'load' , target : 'open ' , time_date : new Date()};
        localStorage.setItem("windowload",JSON.stringify(val));
